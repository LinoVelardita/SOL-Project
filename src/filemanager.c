...
int fs_request_manager(FileSystem_t * fs, int clientFd, int requestType){
    ...
    //switch-case controlla che tipo di richiesta ho ricevuto
    switch (requestType){
        ...
        case WRITE_F:;{
            ...
            if (readn(clientFd, path, MAX_PATH) != MAX_PATH) return -1;  // path
            ASSERT_EXIT(buf = malloc(size), == NULL);
...
int openFile_handler(FileSystem_t * fs, int clientFd, char * path, int flags){
    ...
    ASSERT_EXIT(pthread_mutex_lock(&fs->fs_lock), != 0);    //lock su fs

    logs("The %d client requested an open on the %s file with flag %d", clientFd, path, flags);
    ...
    if(file == NULL){     //file non trovato
        ...
        if(fs->actNumFile + 1 > fs->maxNumFile){   // parte l'algoritmo della cache 
            ...
            if (writen(clientFd, &retVal, sizeof(int)) != sizeof(int)) perror("files will be lost");

            logs("Sending the %s file of %d bytes to the %d client", clientFd, tmp->path, tmp->size);
        
            if (writen(clientFd, &tmp->size, sizeof(int)) != sizeof(int))perror("files will be lost"); 
            if (writen(clientFd, tmp->path, MAX_PATH) != MAX_PATH) perror("files will be lost"); 
            if (writen(clientFd, tmp->cont, tmp->size) != tmp->size) perror("files will be lost"); 
        ...
        insert_l(&newfile->openedBy, clientFd);
        f_doneWrite(newfile);
        fs->actNumFile++;
    ...
    if(file != NULL){ // il file esiste
        ...
        if(flags == 0){ // nessun flag
            f_startWrite(file);
            insert_l( &file->openedBy, clientFd);
            f_doneWrite(file);  
            retVal = 0;   
            goto openFile_handler_END;   
        ...
        if (flags == 2){    // O_LOCK
            ...
            if(file->lockedBy == 0){
                ...
                insert_l(&file->openedBy, clientFd);
                f_doneWrite(file);//sincronizzazione
                retVal = 0;   
                goto openFile_handler_END;   
            }
            else{
    ...
    logs("The client %d has completed the request. Returned value: %d", clientFd, retVal);
    ASSERT_EXIT(pthread_mutex_unlock(&fs->fs_lock), != 0);   
...
int writeFile_handler(FileSystem_t * fs, int clientFd, char* path, int size, char * buf){
    ...
    ASSERT_EXIT(pthread_mutex_lock(&fs->fs_lock), != 0);    // prendo lock fs
    
    logs("The %d client requested a write to the %s file of %d bytes.", clientFd, path, size);
    ...
    f_startRead(file); //f_startread -> sicroninizzazione
    
	//mem_l -> datastructure.c
    if(!mem_l(&file->openedBy, clientFd)){    // controllo se ho aperto il file
        f_doneRead(file);
        retVal = E_NOT_OPN;
        goto writeFile_handler_END;
    }
    ...
    while((fs->maxSize - fs->actSize < size)){  // se lo spazio libero è minore di quello che devo inserire
        ...
        logs("Sending to the %d client the %s file of %d bytes", clientFd, tmp->path, tmp->size);
        
        if (writen(clientFd, &tmp->size, sizeof(int)) != sizeof(int))perror("files will be lost"); 
        if (writen(clientFd, tmp->path, MAX_PATH) != MAX_PATH) perror("files will be lost"); 
        if (writen(clientFd, tmp->cont, tmp->size) != tmp->size) perror("files will be lost"); 

        logs("File %s deleted from the server", tmp->path);
        deleteFile(tmp);
...
int readNFiles_handler(FileSystem_t * fs, int clientFd, int numFile){
    ...
    logs("The %d client requested the reading of %d files", clientFd, numFile);
...
int readFile_handler(FileSystem_t * fs, int clientFd, char * path){
    ...
    logs("The %d client requested the reading of %s", clientFd, path);
    ...
    ASSERT_EXIT(pthread_mutex_unlock(&fs->fs_lock), != 0); // posso rilaciare la mutex del fs

    if(!mem_l(&file->openedBy, clientFd)){ retVal = E_NOT_OPN; f_doneRead(file); goto readFile_handler_END;}
    ...
    logs("Sending to the %d client the %s file of %d bytes", clientFd, file->path, file->size);
...
int removeFile_handler(FileSystem_t * fs, int clientFd, char * path){  
    ...
    logs("The %d client requested the deletion of %s", clientFd, path);
...
int closeFile_handler(FileSystem_t * fs, int clientFd, char * path){
    ...
    logs("The %d client requested the closure of %s", clientFd, path);
    ...
    remove_l(&file->openedBy, clientFd);

    f_doneWrite(file);

    retVal = 0;
    closeFile_handler_END:
    logs("Il client %d ha completato la richiesta. Valore restituito: %d", clientFd, retVal);
    ASSERT_EXIT(pthread_mutex_unlock(&fs->fs_lock), != 0);
    if (writen(clientFd, &retVal, sizeof(int)) != sizeof(int)) return -1;
    else return 0;
...
int unlockFile_handler(FileSystem_t * fs, int clientFd, char * path){
    ...
    logs("The %d client requested an unlock on %s", clientFd, path);
    ...
    if(!mem_l(&file->openedBy, clientFd)){ f_doneWrite(file), retVal = E_NOT_OPN; goto unlockFile_handler_END;}
...
int closeAll_handler(FileSystem_t * fs, int clientFd){
    ...
    ASSERT_EXIT(pthread_mutex_lock(&fs->fs_lock), != 0);

    logs("Close the %d client and all open files", clientFd);

    File_t * next,* tmp = fs->firstFile;

    while(tmp){
        f_startWrite(tmp);
        remove_l(&tmp->openedBy, clientFd);
...
int lockFile_handler(FileSystem_t * fs, int clientFd, char * path){
    ...
    logs("The %d client requested a lock on %s", clientFd, path);
    ...
    if(!mem_l(&file->openedBy, clientFd)){ f_doneWrite(file), retVal = E_NOT_OPN; goto lockFile_handler_END;}
...
int appendToFile_handler(FileSystem_t * fs,int clientFd, char * path, char * buf, int size){
    ...
    logs("The %d client has requested an append on the %s file of %d bytes.", clientFd, path, size);
    ...
    if(!mem_l(&file->openedBy, clientFd)){f_doneRead(file); retVal = E_NOT_OPN; goto appendToFile_handler_END;}
    ...
    ASSERT_EXIT(file->cont = realloc(file->cont, file->size + size), == NULL);
...
File_t * cacheEvict(FileSystem_t * fs, File_t * f, int flag){ //POLITICA FIFO
    // assume la lock sul fs,
    ...
    while(tmp != NULL){
        ...
        if(tmp != f && ((tmp->size != 0) || !flag)){ // deve essere diverso dal file che sto scrivendo
            ...
            fs->evictCount++;
            return tmp; //ritorna il puntatore del file buttato via
    ...
    ASSERT_EXIT("evict fail", );    // per le precondizioni della chiamata, l'evict non dovrebbe mai fallire
    return NULL;
}

void deleteFile(File_t * tmp){
    if(tmp->cont != NULL) free(tmp->cont);
    destroy_l(&tmp->openedBy);
    ASSERT_EXIT(pthread_mutex_destroy(&tmp->f_mutex), != 0);
    ASSERT_EXIT(pthread_mutex_destroy(&tmp->f_order), != 0);
    ASSERT_EXIT(pthread_cond_destroy(&tmp->f_go), != 0);
    free(tmp);
}
...
FileSystem_t * init_FileSystem(int maxNumFile, int maxSize){
    ...
    FileSystem_t * fs;
    ASSERT_EXIT(fs = malloc(sizeof(FileSystem_t)), == NULL);
    ...
    ASSERT_EXIT(pthread_mutex_init(&fs->fs_lock, NULL), != 0);
...
File_t * init_File(char* path, int clientFd){
    ...
    File_t * file;
    ASSERT_EXIT(file = malloc(sizeof(File_t)), == NULL);

    
    ASSERT_EXIT( strncpy(file->path, path, MAX_PATH), == NULL);
    file->size = 0;
    file->cont = NULL;
    file->prev = NULL;
    file->next = NULL;
    file->openedBy = NULL;
    insert_l(&file->openedBy, clientFd);
    ASSERT_EXIT(pthread_mutex_init(&file->f_mutex, NULL), != 0);
    ASSERT_EXIT(pthread_mutex_init(&file->f_order, NULL), != 0);
    ASSERT_EXIT(pthread_cond_init(&file->f_go, NULL), != 0);
...
void f_startRead(File_t * file){
    ASSERT_EXIT(pthread_mutex_lock(&file->f_order), != 0); // sono in fila
    ASSERT_EXIT(pthread_mutex_lock(&file->f_mutex), != 0); // sono in sezione critica
    while(file->f_activeWriters > 0){     // se ci sono scrittori attivi mi sospendo (ma mantengo il posto in fila)
        ASSERT_EXIT(pthread_cond_wait(&file->f_go, &file->f_mutex), != 0 );
    }
    file->f_activeReaders++;
    ASSERT_EXIT(pthread_mutex_unlock(&file->f_order), != 0); // rilascio il posto in fila
    ASSERT_EXIT(pthread_mutex_unlock(&file->f_mutex), != 0); // rilascio la mutex
}

void f_doneRead(File_t * file){
    ASSERT_EXIT(pthread_mutex_lock(&file->f_mutex), != 0);
    file->f_activeReaders--;
    if(file->f_activeReaders == 0){
        ASSERT_EXIT( pthread_cond_signal(&file->f_go), != 0);   // faccio entrare il prossimo in fila
    }
    ASSERT_EXIT( pthread_mutex_unlock(&file->f_mutex), != 0);
}

void f_startWrite(File_t * file){
    ASSERT_EXIT(pthread_mutex_lock(&file->f_order), != 0); // sono in fila
    ASSERT_EXIT(pthread_mutex_lock(&file->f_mutex), != 0 ); // sono in sezione critica
    while(file->f_activeReaders > 0 || file->f_activeWriters > 0){     // se ci sono scrittori o lettori attivi mi sospendo (ma mantengo il posto in fila)
        ASSERT_EXIT(pthread_cond_wait(&file->f_go, &file->f_mutex), != 0 );
    }
...
void f_doneWrite(File_t * file){
    ASSERT_EXIT(pthread_mutex_lock(&file->f_mutex), != 0);
    file->f_activeWriters--;
    ASSERT_EXIT( pthread_cond_signal(&file->f_go), != 0);
    ASSERT_EXIT( pthread_mutex_unlock(&file->f_mutex), != 0);
}
...
void stats(FileSystem_t * fs){
    ...
    printf("Maximum memory occupation: %d\n", fs->maxRSize);
    printf("Number of cashed evicts: %d\n", fs->evictCount);
    printf("There are %d files in the server", fs->actNumFile);
    if(fs->actNumFile > 0){
        printf(":");
        printFs(fs);
        printf("\n");
